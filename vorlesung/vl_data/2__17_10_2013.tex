...
\definition{}
\begin{itemize}
\item Eine Formel $G$ heißt gültig oder \Index{Tautologie}, falls $f(G) = W$ für jede Interpretation $f$
\item Eine Formel $G$ heißt erfüllbar, falls es eine Interpretation $f$ gibt mit $f(G) = W$
\item Eine Formel $G$ heißt nicht erfüllbar, unerfüllbar oder \Index{Antilogie}, falls $f(G) = F$ für alle Interpretationen $f$
\end{itemize}
...
\\
%\begin{tabbing}
\Index{Assoziativität} \> $((G \AND H) \AND I ) \equiv (G \AND (H \AND I))$\\
\> $((G \OR H) \OR I) \equiv (G \OR (H \OR I ))$\\
%\end{tabbing}
\\
%\begin{tabbing}
\Index{Distributivität} \> $(G \AND ( H \OR I )) \equiv ((G \AND H ) \OR (G \AND I ))$\\
\> $(G \OR ( H \AND I )) \equiv (( G \OR H ) \AND ( G \OR I ))$\\
%\end{tabbing}
\\
%\begin{tabbing}
\Index{Absorption} \> $(G \AND ( G \OR H )) \equiv G$\\
\> $(G \OR ( G \AND H )) \equiv G$\\
%\end{tabbing}
\\
\Index{Doppelnegation} \> $\neg \neg G \equiv G$\\
\\
%\begin{tabbing}
\Index{deMorgan} Regel \> $\neg ( G \AND H ) \equiv \neg G \OR \neg H$\\
\> $\neg ( G \OR H) \equiv \neg G \AND \neg H$
\end{tabbing}
